...
ここまではまだ、あなたのパソコンの中だけで動いている状態です。
ichibom鯖のみんなにDiscordでURLを共有して使ってもらうには、次の
STEP 11で「公開(デプロイ)」する必要があります。

---

## STEP 11. Discordでみんなに共有できるように公開する(デプロイ)

**何をするか:** このアプリを、24時間動き続けるサーバー(ホスティング
サービス)にアップロードし、誰でもアクセスできるURL(例:
`https://ichibom-stream-portal.onrender.com`)を発行します。
そのURLをDiscordに貼れば、ichibom鯖のメンバー全員が自分のスマホや
PCから見られるようになります。

**なぜ必要か:** `python app.py` で起動したページは、あなたの
パソコンの中でしか動いていません(`localhost:5000` は「自分のPC」を
指すアドレスです)。パソコンの電源を切ったり、Wi-Fiを切ったりすると
即座に見られなくなりますし、そもそも他の人のスマホやPCからは
アクセスする方法がありません。「みんなが見られるURL」にするには、
インターネット上で動き続けるサーバーが必要です。

ここでは無料枠のある **Render** を使う手順を説明します
(他に PythonAnywhere なども選べますが、Renderの方が今回のような
Flaskアプリを一番簡単にデプロイできます)。

### 11-1. gunicornを使う準備(すでに設定済み)

`python app.py` で使っている `app.run(debug=True)` は、開発中に
自分だけで確認するための簡易サーバーで、複数人が同時にアクセスする
本番環境には向いていません。そこで本番では **gunicorn** という
本番向けのサーバーを使います。`requirements.txt` にはすでに
`gunicorn` を追加してあるので、この手順で追加の作業は不要です。

### 11-2. GitHubリポジトリを最新の状態にする

Renderは「GitHubリポジトリを見て、その内容を自動でデプロイする」
仕組みです。ローカルで `streamers.py` などを編集した場合は、
Renderにデプロイする前に必ずGitHubへpushしておいてください。
...
### 11-3. Renderでアカウント作成 & Web Serviceを作成する

1. https://render.com にアクセスし、GitHubアカウントでサインアップ
2. ダッシュボードで「New +」→「Web Service」を選択
3. 「Build and deploy from a Git repository」を選び、
   `ichibom_stream_portal` リポジトリを選択(連携許可が必要な場合は
   画面の指示に従ってください)
4. 設定項目を入力
   - **Name**: 好きな名前(例: `ichibom-stream-portal`)。ここが
     そのままURLの一部になります
   - **Build Command**: `pip install -r requirements.txt`
   - **Start Command**: `gunicorn app:app`
   - **Instance Type**: `Free` を選択

### 11-4. 環境変数(Client ID/Secret)を設定する

`.env` ファイルはGitHubにアップロードされていない(`.gitignore`で
除外済み)ので、Render側に改めて設定する必要があります。

「Environment」タブ(または作成画面の「Environment Variables」)で、
`.env` に書いたのと同じ内容をひとつずつ追加します。

| Key | Value |
|---|---|
| `TWITCH_CLIENT_ID` | (あなたのClient ID) |
| `TWITCH_CLIENT_SECRET` | (あなたのClient Secret) |
| `CACHE_TTL_SECONDS` | `30` |

**なぜ必要か:** `.env` はあなたのパソコンの中だけにある機密情報です。
Renderのサーバー上でTwitch APIに接続するには、Render自身にも
同じ情報を(安全な方法で)渡す必要があります。ここに入力した値は
GitHub上のコードには一切含まれず、Renderが管理者権限を持つ人にしか
見えない形で保存されます。

### 11-5. デプロイして公開URLを確認する

「Create Web Service」(または「Deploy」)を押すと、自動でビルドが
始まります。ログに `Build successful` のような表示が出て、
画面上部に緑色の `Live` と表示されれば公開完了です。

ページ上部に表示される `https://(あなたのアプリ名).onrender.com`
のようなURLにブラウザでアクセスし、ローカルで確認したのと同じ画面が
表示されればOKです。

**確認方法:** 発行されたURLに、別のスマホやPC(自分のパソコン以外の
端末)からアクセスしてみて、同じように表示されればみんなも見られる
状態になっています。

**エラーが出たら:**
- ページが表示されない/エラー画面が出る → Renderダッシュボードの
  「Logs」タブを開き、`ValueError: TWITCH_CLIENT_ID...` のような
  エラーが出ていないか確認してください(→ 環境変数の設定漏れが
  多い原因です)。
- 最初のアクセスがすごく遅い(30秒以上かかる) → Render無料枠は
  一定時間アクセスがないとサーバーが休止(スリープ)し、次の
  アクセスで起動し直すため、最初の1回だけ時間がかかります。
  ichibom鯖の人数規模であれば、実用上は問題になりにくいはずです。
  気になる場合は有料プランへのアップグレードで解消できます。

### 11-6. Discordに貼る

あとは発行されたURLをichibom鯖のDiscordチャンネルに貼るだけです。
`streamers.py` を更新してGitHubにpushすると、Renderが自動的に
再デプロイして内容を反映してくれます(手動での再デプロイ操作は
基本的に不要です)。
...
- 24時間動き続けるURLが発行されている ✅
- 自分のパソコン以外の端末からもアクセスできる ✅
- Client ID/SecretはRenderの環境変数として設定し、コードには含まれていない ✅
- DiscordにURLを共有できる状態になっている ✅
...
ichibom鯖のみんなに公開する具体的な手順はSTEP 11にまとめてあります。
ポイントだけ再掲すると、

- `app.run(debug=True)` は開発用です。公開する場合は `gunicorn` などの
  本番向けサーバー経由で動かします(STEP 11で設定済み)。
- `.env` の中身は絶対に公開リポジトリにコミットしないでください。
  デプロイ先には環境変数として別途設定します。
